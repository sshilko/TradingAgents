...
from cli.report_scroller import KeyReader, ReportScroller, ScrollableReport
...
# Scroll offset for the "Current Report" panel, driven by the arrow keys.
report_scroller = ReportScroller()
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    ...
    if message_buffer.current_report:
        # Re-rendered on every live refresh, so a key press shows up without a
        # layout update. Scroll with Up/Down, Page Up/Page Down, Home/End.
        layout["analysis"].update(
            ScrollableReport(
                lambda: message_buffer.current_report,
                report_scroller,
...
def run_analysis(checkpoint: bool = False):
    ...
    report_scroller.reset()

    # The key reader owns stdin for the run, so it starts before the live
    # display and restores the terminal when the run ends.
    with (
        KeyReader(report_scroller.queue_key) as keys,
        Live(layout, refresh_per_second=4) as live,
    ):
        # Initial display
